...
import com.codingbingoZTT.BanmaReader.manager.SettingManager;
...
import com.codingbingoZTT.BanmaReader.utils.FileUtils;
import com.codingbingoZTT.BanmaReader.utils.ScreenUtils;
import com.codingbingoZTT.BanmaReader.utils.StringUtils;
import com.codingbingoZTT.BanmaReader.utils.ThreadPool;
...
public class PageFactory {
    ...
    public static List<String> getmTTSLines() {
        return mTTSLines;
    }

    public static void setmTTSLines(List<String> mTTSLines) {
        PageFactory.mTTSLines = mTTSLines;
    }

    public static  List<String> mTTSLines = new ArrayList<>();
    ...
    private static volatile PageFactory instance;

    //定义一个共有的静态方法，返回该类型实例
    public static PageFactory getIstance(Context mContext) {
        ...
        if (instance == null) {
            ...
            synchronized (PageFactory.class) {
                //未初始化，则初始instance变量
                if (instance == null) {
                    instance = new PageFactory(mContext);
                }
            }
        }
        return instance;
    ...
    public synchronized List<String> onDraw(Canvas canvas) {
        if (mLines.size() == 0) {
    ...
    public interface listener {
        public List<String> solve(List<String> result);
    }

    public  List<String> TTSCurrentLines() {
            mLines = pageDown();
        return mLines;
    }
}
